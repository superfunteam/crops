...
The agent supplies `CROPS_ACCESS_KEY` privately and posts to `/api/hooks` with Bearer authentication. Reports use `action: log` and zero duration, preserving human hours and active timers. Non-billable is the default; users can explicitly include reported cost in billing. Instructions report tokens alongside observed subscription caps, reset windows, and remaining capacity. Cost is the work’s allocated slice of the actual subscription fee using a user-approved, period-compatible capacity budget—not API token pricing. Unknown plan/cap data is labeled unknown; missing cost-allocation inputs keep reports pending. Shared ledgers prevent overlapping allocations and allocations above the subscription fee. Instructions distinguish measured usage from estimates and explain Crops’ 24-hour idempotency retention. No plan prices or secrets are embedded in the generated prompt.

The prompt builder also accepts provider/plan, actual USD subscription fee, billing-period dates, individual/shared scope, and an active-agent-hours capacity budget. These non-secret details are saved in this browser and included in the copied instructions; the agent reuses them without reasking. Blank setup reuses prior agent context or asks once for missing details. Unavailable cap telemetry is reported as unknown and does not block a valid allocation. Access keys remain separate and private.
